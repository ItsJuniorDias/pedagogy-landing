...
const FATIGUE = { graceSec: 75, rampSec: 150, slowFrac: 0.34, errorAdd: 0.5, catchShrink: 0.2, tiredAt: 0.5 }
const AI = { spinBlind: 0.35, errBase: 0.65, errBySpeed: 0.5 }
const RALLY = { baseBoost: 1.06, cutBoost: 0.02, cutBoostMax: 0.14, maxMul: 2.3 }
const DIFFS = {
  easy: { id: 'easy', label: 'EASY', emoji: '🐢', color: NEON.mint, aiSpeed: 2.2, ballSpeed: 3.2, aiError: 0.6, aiCatch: 0.18 },
  normal: { id: 'normal', label: 'NORMAL', emoji: '⚡', color: NEON.amber, aiSpeed: 3.2, ballSpeed: 4.2, aiError: 0.34, aiCatch: 0.28 },
  hard: { id: 'hard', label: 'HARD', emoji: '🔥', color: NEON.rose, aiSpeed: 4.3, ballSpeed: 5.2, aiError: 0.18, aiCatch: 0.4 },
}
...
function Racket({ rubber, edge, innerRef }) {
  ...
  return (
    <group ref={innerRef}>
      ...
      <mesh position={[0, bladeY - R - 0.02, 0.06]} rotation-x={Math.PI / 2 - 0.5}>
...
function Table() {
  ...
  return (
    <group>
      ...
      {[-1, 1].map((s) => (
        <mesh key={s} position={[s * (TABLE.halfW + 0.15), TABLE.topY + 0.17, 0]}>
      ...
      {/* glass side walls */}
      {[-1, 1].map((s) => (
        <mesh key={s} position={[s * (TABLE.halfW + 0.04), TABLE.topY + 0.21, 0]}>
...
function Arena({ orbsRef }) {
  ...
  const orbs = useMemo(() => {
    ...
    for (let i = 0; i < 14; i++) {
      ...
      } while (Math.abs(x) < TABLE.halfW + 1.2 && Math.abs(z) < TABLE.halfL + 1.2)
      out.push({
        x,
        z,
  ...
  return (
    <group>
      ...
      <gridHelper args={[28, 28, C3D.gridA, C3D.gridB]} position={[0, floorY + 0.09, 0]} />
      {orbs.map((o, i) => (
        <mesh
          key={i}
...
// =============================================================================
function Scene({ g, diffRef, setPhase, setSpeedMul, spawnLabel, onPointRef, onHitRef }) {
  ...
  useFrame((_, delta) => {
    ...
    orbsRef.current.forEach((orb) => {
      if (orb) orb.position.y = orb.userData.baseY + 0.18 * Math.sin(r.t * orb.userData.spd + orb.userData.off)
    })
    ...
    if (r.phase === 'play') {
      ...
      r.playTime += dt
      const fatigue = clamp((r.playTime - FATIGUE.graceSec) / FATIGUE.rampSec, 0, 1)
      const effAiSpeed = diffCfg.aiSpeed * (1 - fatigue * FATIGUE.slowFrac)
      const effAiCatch = Math.max(0, diffCfg.aiCatch - fatigue * FATIGUE.catchShrink)
      ...
      ai.position.x = clamp(ai.position.x + clamp(dx, -maxMove, maxMove), -limX, limX)
      ...
      const tryPaddle = (paddle, dirSign, catchAssist) => {
        ...
        if (Math.abs(hitX - paddle.position.x) > PADDLE.halfW + BALL_R + catchAssist) return

        const paddleVX = paddle.userData.vx ?? 0
        const boost = RALLY.baseBoost + Math.min(RALLY.cutBoostMax, Math.abs(paddleVX) * RALLY.cutBoost)
  ...
  return (
    <>
      ...
      <directionalLight
        ...
        shadow-bias={-0.0005}
      />
      <pointLight color={C3D.player} intensity={9} distance={16} position={[-4, 3, 5]} />
      <pointLight color={C3D.ai} intensity={9} distance={16} position={[4, 3, -5]} />

      <Arena orbsRef={orbsRef} />
      <Table />

      <group position={[0, 0, PADDLE.z]}>
        <Racket rubber={C3D.rubberPlayer} edge={C3D.player} innerRef={playerRef} />
      </group>
      <group position={[0, 0, -PADDLE.z]} rotation-y={Math.PI}>
        <Racket rubber={C3D.rubberAi} edge={C3D.ai} innerRef={aiRef} />
      </group>

      <Trail width={0.5} length={4} color={NEON.yellow} attenuation={(t) => t * t} decay={1} local>
...
function Scoreboard({ score, rally, bestRally, speedMul }) {
  ...
  const Side = ({ name, value, color }) => (
    <div style={{ width: 96, textAlign: 'center' }}>
      <div style={{ width: 6, height: 6, borderRadius: 3, background: color, margin: '0 auto 3px' }} />
      <div style={{ fontFamily: FONT, fontSize: 10, letterSpacing: 1.4, color }}>{name}</div>
      <div style={{ fontFamily: FONT, fontSize: 40, lineHeight: '44px', color, textShadow: `0 0 16px ${color}88` }}>{value}</div>
    </div>
  )
  return (
    <div style={{ position: 'absolute', top: 16, left: 16, right: 16, display: 'flex', justifyContent: 'center', pointerEvents: 'none' }}>
      <div style={{ ...glass, borderRadius: 22, padding: '10px 18px 9px', width: 'min(440px, 100%)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <Side name="YOU" value={score.p} color={NEON.cyan} />
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontFamily: FONT, fontSize: 11, letterSpacing: 3, color: NEON.dim }}>VS</div>
            <div style={{ fontFamily: FONT, fontSize: 14, color: NEON.text }}>🔥 {rally}</div>
            <div style={{ fontFamily: FONT, fontSize: 9, letterSpacing: 1.5, color: NEON.dim }}>BEST {bestRally}</div>
          </div>
          <Side name="CPU" value={score.c} color={NEON.magenta} />
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 8 }}>
          <span style={{ fontFamily: FONT, fontSize: 9, letterSpacing: 2, color: NEON.dim }}>SPD</span>
          <div style={{ flex: 1, height: 4, borderRadius: 2, background: 'rgba(125,140,180,0.25)', overflow: 'hidden' }}>
            <div style={{ height: '100%', width: `${3 + progress * 97}%`, background: NEON.yellow, borderRadius: 2, transition: 'width .35s ease-out', boxShadow: `0 0 8px ${NEON.yellow}` }} />
          </div>
          <span style={{ fontFamily: FONT, fontSize: 10, color: NEON.yellow }}>{speedMul.toFixed(2)}x</span>
        </div>
      </div>
    </div>
  )
...
function ControlBar({ phase, diff, onTogglePause, onReset, onSelectDiff }) {
  ...
  const Btn = ({ children, onClick, disabled, w = 44 }) => (
    <button
      ...
      disabled={disabled}
      style={{
        width: w, height: 44, borderRadius: 22, border: 'none', background: 'transparent',
        color: NEON.text, fontSize: 19, cursor: disabled ? 'default' : 'pointer', opacity: disabled ? 0.35 : 1,
      }}
    >
      {children}
    </button>
  )
  const divider = <div style={{ width: 1, height: 28, background: 'rgba(125,140,180,0.25)' }} />
  return (
    <div style={{ position: 'absolute', bottom: 16, left: 16, right: 16, display: 'flex', justifyContent: 'center' }}>
      <div style={{ ...glass, borderRadius: 26, padding: '7px 8px', display: 'flex', alignItems: 'center', gap: 6, width: 'min(440px, 100%)' }}>
        <Btn onClick={onTogglePause} disabled={pauseDisabled}>{phase === 'paused' ? '▶️' : '⏸'}</Btn>
        {divider}
            ...
            return (
              <button
                ...
                style={{
                  ...
                  padding: '6px 0', borderRadius: 16, cursor: 'pointer',
                  background: active ? d.color + '33' : 'transparent',
                  border: `1px solid ${active ? d.color : 'transparent'}`,
                }}
              >
                <span style={{ fontSize: 15 }}>{d.emoji}</span>
                <span style={{ fontFamily: FONT, fontSize: 9, letterSpacing: 1.2, color: active ? d.color : NEON.dim }}>{d.label}</span>
              </button>
            )
          })}
        </div>
...
export default function NeonPong() {
  ...
  const g = useRef({
    ball: null, player: null, ai: null, orbs: [],
    vel: { x: 0, z: 0 }, spin: 0, targetX: 0, phase: 'idle',
    serveAt: 0, serveDir: -1, speedMul: 1, rally: 0, t: 0,
    playTime: 0, prevVZSign: 0, aiAimErr: 0, tiredShown: false,
    hitPlayer: 0, hitAi: 0, keys: new Set(),
  })
  ...
  const scheduleServe = useCallback((dir, delay = 900) => {
    const r = g.current
    resetBall()
    r.serveDir = dir
  ...
  const onPoint = useCallback((who) => {
    const isPlayer = who === 'player'
    const prev = scoreRef.current
    const ns = { p: prev.p + (isPlayer ? 1 : 0), c: prev.c + (isPlayer ? 0 : 1) }
    scoreRef.current = ns
    setScore(ns)
    g.current.rally = 0
    setRally(0)
    ...
    g.current.aiAimErr = 0
    g.current.tiredShown = false
    spawnLabel(isPlayer ? '🎉 POINT!' : '🤖 CPU POINT', isPlayer ? NEON.cyan : NEON.magenta)
  ...
  const onHit = useCallback((spinMag, isPlayer) => {
    const r = g.current
    r.rally += 1
    setRally(r.rally)
    ...
    if (isPlayer && spinMag >= SPIN.labelAt) spawnLabel('🌀 EFFECT!', NEON.cyan)
  }, [spawnLabel])
  ...
  useEffect(() => {
    const down = (e) => {
      ...
      if ((k === ' ' || k === 'enter') && g.current.phase === 'idle') { startGame(); return }
  ...
  const HINT = { idle: 'FIRST TO 7 POINTS', serving: 'GET READY…', play: 'DRAG TO MOVE YOUR PADDLE', paused: 'PAUSED', over: 'MATCH OVER' }
  const playerWon = score.p >= WIN_SCORE

  return (
    <div ref={wrapRef} onPointerMove={onPointerMove} onPointerDown={onPointerDown}
      style={{ position: 'relative', width: '100%', height: '100%', background: NEON.bg, overflow: 'hidden', touchAction: 'none' }}>
      <style>{`
      ...
      <Canvas shadows dpr={[1, 2]} camera={{ position: [0, 5.6, 9.0], fov: 56, near: 0.1, far: 100 }}>
        <Scene
          g={g}
          diffRef={diffRef}
          setPhase={setPhase}
      ...
      <Scoreboard score={score} rally={rally} bestRally={bestRally} speedMul={speedMul} />

      {/* floating labels */}
      <div style={{ position: 'absolute', top: '42%', left: 0, right: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', pointerEvents: 'none' }}>
        {labels.map((l) => (
          <div key={l.id} style={{ position: 'absolute', fontFamily: FONT, fontSize: 22, letterSpacing: 1.5, color: l.color, textShadow: `0 0 12px ${l.color}aa, 0 1px 4px rgba(0,0,0,.6)`, animation: 'npFloat 1.3s ease-out forwards', whiteSpace: 'nowrap' }}>
            {l.text}
          </div>
        ))}
      </div>
      ...
      {phase === 'idle' && (
        <div style={{ position: 'absolute', inset: 0, display: 'grid', placeItems: 'center' }}>
          <button onClick={startGame} style={{ ...glass, border: `1px solid ${NEON.cyan}88`, borderRadius: 28, padding: '22px 38px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, cursor: 'pointer', animation: 'npPulse 1.4s ease-in-out infinite' }}>
            <span style={{ fontSize: 36 }}>🏓</span>
            <span style={{ fontFamily: FONT, fontSize: 22, letterSpacing: 3, color: NEON.cyan, textShadow: `0 0 16px ${NEON.cyan}` }}>TAP TO PLAY</span>
            <span style={{ fontFamily: FONT, fontSize: 10, letterSpacing: 1.5, color: NEON.dim }}>drag your paddle · first to 7</span>
          </button>
        </div>
      )}

      {/* hint */}
      <div style={{ position: 'absolute', bottom: 86, left: 0, right: 0, textAlign: 'center', pointerEvents: 'none', fontFamily: FONT, fontSize: 11, letterSpacing: 2.5, color: NEON.dim, animation: 'npPulse 1.4s ease-in-out infinite' }}>
        {HINT[phase]}
      </div>

      <ControlBar phase={phase} diff={diff} onTogglePause={togglePause} onReset={resetGame} onSelectDiff={setDiff} />

      {/* game over */}
      {overVisible && (
        <div style={{ position: 'absolute', inset: 0, display: 'grid', placeItems: 'center', background: 'rgba(11,16,38,0.6)', backdropFilter: 'blur(4px)' }}>
          <div style={{ ...glass, borderRadius: 28, padding: '28px 34px', textAlign: 'center', animation: 'npPop .25s ease-out', maxWidth: 320 }}>
            <div style={{ fontSize: 52 }}>{playerWon ? '🏆' : '🤖'}</div>
            <div style={{ fontFamily: FONT, fontSize: 26, letterSpacing: 2, color: playerWon ? NEON.cyan : NEON.magenta, textShadow: `0 0 16px ${(playerWon ? NEON.cyan : NEON.magenta)}88` }}>
            ...
            <div style={{ fontFamily: FONT, fontSize: 11, letterSpacing: 1.5, color: NEON.dim, marginTop: 4 }}>🔥 BEST RALLY {bestRally}</div>
            <button onClick={startGame} style={{ marginTop: 18, ...glass, border: `1px solid ${NEON.cyan}88`, borderRadius: 18, padding: '12px 28px', fontFamily: FONT, fontSize: 15, letterSpacing: 2, color: NEON.cyan, cursor: 'pointer' }}>
              PLAY AGAIN
            </button>
          </div>
        </div>
