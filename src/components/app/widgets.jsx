  ...
  return (
    <Reveal delay={delay}>
      ...
      >
        ...
        <div className="mt-3 font-display font-extrabold text-3xl text-ink leading-none">{value}</div>
...
export function WeekBars({ week = [], color = 'grape' }) {
  const c = resolveColor(color)
  ...
  return (
    <div className="rounded-3xl bg-white p-5 ring-1 ring-black/5 shadow-[0_14px_34px_-26px_rgba(58,49,66,.6)] h-full">
      <div className="flex items-baseline justify-between">
        <h3 className="font-display font-extrabold text-lg text-ink">This week</h3>
        <span className="text-[13px] font-bold text-inksoft">
          {week.reduce((a, b) => a + b, 0)} min read
        </span>
      </div>
        ...
        {week.map((v, i) => {
          ...
          return (
            <div key={i} className="flex flex-col items-center gap-2 flex-1">
              ...
              <span
                className={
                  'text-[11px] font-extrabold ' + (isToday ? 'text-grape' : 'text-inksoft/70')
                }
              >
                {WEEKDAYS[i]}
              </span>
...
export function PathTile({ name, emoji, done, total, color = 'grape', delay = 0 }) {
  const c = resolveColor(color)
    ...
    <Reveal delay={delay}>
      ...
      >
        <div className="flex items-center gap-3">
          <span className="grid place-items-center w-12 h-12 rounded-2xl text-2xl" style={{ background: c.bg }}>
            {emoji}
          </span>
          <div className="min-w-0">
            <div className="font-display font-extrabold text-ink truncate">{name}</div>
