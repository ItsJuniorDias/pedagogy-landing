    ...
    return (
      <Reveal className="mt-10 text-center max-w-md mx-auto">
        ...
        <Link
          to="/app/profile"
          state={{ add: true }}
          className="btn3d b-pink px-6 py-3.5 mt-6 inline-flex"
        >
          ＋ Add a reader
        </Link>
      </Reveal>
    ...
    <div className="space-y-8">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-1.5 h-1.5 rounded-full bg-cream overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${Math.round((resume.done / resume.total) * 100)}%`,
      ...
      <Reveal>
        ...
        >
          <div className="relative z-10 max-w-md">
            ...
            <Link
              to="/app/stories"
              className="btn3d b-sun px-5 py-3 mt-4 inline-flex"
            >
              🔍 Explore now
            </Link>
          </div>
          <div className="pointer-events-none absolute -right-4 -bottom-6 text-[8rem] opacity-90 select-none">
            🪐
          </div>
          <div className="pointer-events-none absolute right-24 top-6 text-3xl opacity-80 select-none">
            ⭐
          </div>
        </motion.div>
      </Reveal>
        ...
        {QUICK.map((q) => (
          <RevealItem key={q.to} variants={fadeUp}>
            <Link to={q.to}>
              ...
              >
                <span
                  className="grid place-items-center w-12 h-12 mx-auto rounded-2xl text-2xl"
      ...
      <section className="grid lg:grid-cols-2 gap-4 sm:gap-5 items-stretch">
        <Reveal>
          <WeekBars
            week={m.week}
            total={m.minutesThisWeek}
            color={activeReader.color}
          />
        </Reveal>
        <div>
          <SectionHead
            ...
            action={
              <Link
                to="/app/path"
                className="font-display font-extrabold text-grape text-sm hover:underline"
              >
                View all
              </Link>
            }
