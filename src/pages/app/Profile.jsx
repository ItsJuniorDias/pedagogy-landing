    ...
    <div className="space-y-5">
      <div className="flex items-center gap-4 rounded-2xl bg-cream/60 p-4 ring-1 ring-black/5">
        ...
        <div className="min-w-0">
          ...
          <p className="text-sm font-bold text-inksoft truncate">
            {draft.email}
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col-reverse sm:flex-row gap-3 pt-1">
        <button
          onClick={onCancel}
          className="btn3d b-white ring-1 ring-black/5 px-5 py-3 flex-1"
        >
          Cancel
        </button>
        <motion.button
  ...
  return (
    ...
    >
      ...
      >
        ...
        />
        <div className="min-w-0">
          <div className="font-display font-extrabold text-lg text-ink truncate">
            {reader.name}
          </div>
      ...
      <div className="mt-4 flex gap-2">
        ...
        <button
          onClick={onEdit}
          className={
            "btn3d b-white ring-1 ring-black/5 px-4 py-2.5 text-sm " +
            (active ? "flex-1" : "")
          }
        >
          ✏️ Edit
          ...
          <div className="flex flex-wrap items-center gap-5">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-sm font-bold text-inksoft mt-1">
                Managing {readers.length} reader
                {readers.length === 1 ? "" : "s"}
              </p>
            </div>
            <button
              onClick={() => setAccountOpen(true)}
              className="btn3d b-white ring-1 ring-black/5 px-5 py-3"
            >
              Edit account
            </button>
          </div>
          ...
          <RevealItem variants={fadeUp}>
            ...
            >
              <span className="text-center">
                ...
                <span className="mt-3 block font-display font-extrabold text-grape">
                  Add a reader
                </span>
              </span>
            </button>
          ...
          <div className="rounded-[28px] overflow-hidden ring-1 ring-black/5 shadow-[0_18px_40px_-30px_rgba(58,49,66,.6)]">
            <div className="bg-gradient-to-br from-grape to-graped p-6 sm:p-7 text-white">
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                  <p className="text-white/80 font-bold text-sm uppercase tracking-wide">
                    Current plan
                  </p>
                  <p className="font-display font-extrabold text-3xl mt-0.5">
                    {user.plan}
                  </p>
                ...
                {user.plan === "Free" && (
                  <Link
                    to="/app/paywall"
                    className="btn3d b-sun px-6 py-3.5 text-lg"
                  >
                    Upgrade
                  </Link>
                )}
                ...
                <div className="mt-6 pt-5 border-t border-black/5 flex flex-wrap items-center justify-between gap-3">
                  <p className="text-[13px] font-semibold text-inksoft">
                    {user.subscription?.cycle
                      ? `Billed ${user.subscription.cycle}`
                      : "Active subscription"}
