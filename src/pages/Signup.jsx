...
export default function Signup() {
  ...
  const [reader, setReader] = useState({ name: '', age: 5, emoji: '🐻', color: 'grape', level: 'explorer' })
  const [err, setErr] = useState('')

  const next = (e) => {
    e.preventDefault()
    if (!account.name.trim() || !account.email.trim() || account.password.length < 4) {
      ...
      footer={
        step === 0 ? (
          <>
            ...
            <Link to="/login" className="text-grape font-extrabold hover:underline">
              Log in
            </Link>
          </>
        ) : (
        ...
        {step === 0 ? (
          ...
          >
            <div className="flex items-center gap-4 rounded-2xl bg-cream/60 p-4 ring-1 ring-black/5">
              <Avatar emoji={reader.emoji} color={reader.color} size={64} selected />
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <Field label="Age">
                <select
                  ...
                  value={reader.age}
                  onChange={(e) => setReader({ ...reader, age: Number(e.target.value) })}
                >
            ...
            <Field label="Reading level">
              <select
                className="w-full rounded-2xl border-2 border-black/5 bg-cream/40 px-4 py-3 font-bold text-ink focus:outline-none focus:border-grape focus:bg-white"
                value={reader.level}
                onChange={(e) => setReader({ ...reader, level: e.target.value })}
              >
                {LEVELS.map((l) => (
