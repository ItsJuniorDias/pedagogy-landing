...
const storyEntries = (reader) => Object.entries(reader?.progress?.stories || {})
const courseEntries = (reader) => Object.entries(reader?.progress?.courses || {})
...
export function storiesReadCount(reader) {
  ...
  for (const [id, entry] of storyEntries(reader)) {
    ...
    if (chapters.length && completedCount(chapters, entry) === chapters.length) n += 1
  }
  return n
}
...
export function lettersLearnedCount(reader) {
  ...
  const scan = (items, entry) => {
    items.forEach((it, i) => {
      ...
      if (letter && isChapterComplete(entry, it, i)) letters.add(String(letter).toUpperCase())
    })
  }
  for (const [id, entry] of storyEntries(reader)) scan(resolveStory(id).chapters, entry)
  for (const [id, entry] of courseEntries(reader)) scan(resolveCourse(id).lessons, entry)
  return letters.size
}
...
export function pickResume(reader) {
  ...
  for (const [id, e] of storyEntries(reader)) {
    ...
    out.push({ title: card.title, emoji: card.emoji, accent: card.accent, to: `/app/stories/${id}`, done, total: chapters.length, ts: e.updatedAt })
  }
  ...
  for (const [id, e] of courseEntries(reader)) {
    ...
    out.push({ title: course.title, emoji: course.emoji, accent: course.accent, to: `/app/path/${id}`, done, total: lessons.length, ts: e.updatedAt })
  }
